...
export default function Search({
  ...
  const containsMatch = (searchStr, productName, productCategory) => {
    return searchStr
      ...
      .reduce((prev, token) => {
        return (
          productName.toLowerCase().includes(token.toLowerCase()) ||
  ...
  useEffect(() => {
    if (filterCategory === "all") {
      ...
      const newFilteredItems = products.filter(
  ...
  useEffect(() => {
    if (searchInput === "" && filteredProducts.length === 0) {
      // first render of page, so display all products
      setSearchResults([...products]);
    } else if (searchInput === "") {
      // nothing searched, so display all filtered products
      setSearchResults([...filteredProducts]);
    } else {
      // determine which products match search input and filter category
      let newSearchResults = filteredProducts.filter((product) =>
        containsMatch(searchInput, product.name, product.category)
      );
      ...
      <div className="filter-products">
        <ul className="filter-options">
          ...
          <li className={setClassName("accessories")}>
            <button onClick={() => setFilterCategory("accessories")}>
              Accessories
            </button>
          </li>
          ...
          <li className={setClassName("clothing")}>
            <button onClick={() => setFilterCategory("clothing")}>
              Clothing
            </button>
          </li>
        </ul>
