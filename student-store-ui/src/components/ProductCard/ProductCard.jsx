            ...
            <div className="single-product-info">
                ...
                <div className='quantity-info'>
                    <div className='button-wrapper'>
                        <AddBoxRoundedIcon className="add" onClick={() => handleAddItemToCart(productId)}/>
                        <IndeterminateCheckBoxRoundedIcon className="remove" onClick={()=> handleRemoveItemFromCart(productId)}/>
                    </div>
                    <div className='product-quantity'>
                        {/* only render the product quantity if product exists */}
                        {quantity === 0 ? <></>: <span className='qty-span'>{quantity}</span>}
                    </div>
                </div>
            </div>
        </div>
