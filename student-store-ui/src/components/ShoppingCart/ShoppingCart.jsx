      ...
      <div className="shopping-cart">
        ...
        <div className="order-table">
          ...
          {shoppingCart.map((cartItem) => {
            return (
              <div
                className="item-and-quantity order-four-cols"
                key={cartItem.itemId}
              >
                <>
                  <span className="cart-product-name">
                    {idToName(cartItem.itemId)}
                  </span>
                  <span className="cart-product-quantity">
                    {cartItem.quantity}
                  </span>
                  <span className="cart-product-unit-price">
                    ${calcUnitPrice(cartItem)?.toFixed(2)}
                  </span>
                  <span className="cart-product-cost">
                    ${(cartItem.quantity * calcUnitPrice(cartItem))?.toFixed(2)}
                  </span>
                </>
              </div>
            );
        ...
        <div className="subtotal-three-rows">
          <div className="subtotal shopping-cols">
            ...
            <h4 className="price">
              ${calcTotalCost(shoppingCart)?.toFixed(2)}
            </h4>
          </div>
